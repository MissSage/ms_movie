...
class Q3BSPLump {
  ...
  public constructor(offset = 0, length = 0) {
    this.offset = offset
    this.length = length
  }
}
...
export class Q3BSPTexture {
  public name = '' //quake bsp中name都是定长字符串，包括null（0）结尾符，共64字节
  public flag = -1 //本Demo没用到
  public content = -1 //本Demo没用到
...
export class Q3BSPSurface {
  public textureIdx = 0 // 4 bytes 指向textures数组中的索引
  public effectIdx = -1 // 4 bytes 本书没有使用该数据
  public faceType: EQ3BSPSurfaceType = EQ3BSPSurfaceType.BAD // 4 bytes
  // 下面的四个变量是理解BSP场景渲染的关键，在Quake3BSPScene中详解！！！
  public firstVertIdx = -1 // 4 bytes 索引指向vertArray中的offset顶点
  public numVert = 0 // 4 bytes 顶点数量  通过firstVertIdx和numVert可以获得vertex数据
  public firstIndex = -1 // 4 bytes 指向索引缓存
  public numIndex = 0 // 4 bytes
  // 下面的数据本书没用使用
  public lightMapIdx = 0 // 4 bytes
  public lightMapX = 0 // 4 bytes
  public lightMapY = 0 // 4 bytes
  public lightMapWidth = 0 // 4 bytes
  public lightMapHeight = 0 // 4 bytes
  ...
  public patchWidth = 0 // 4 bytes
  public patchHeight = 0 // 4 bytes
  // 合计每个表面所占字节数为104
  public static TOTALBYETS = 104 // （12 * 4 + 12 * 4 + 8 = 104个字节）
}

export class Q3BSPModel {
  public mins: vec3 = vec3.zero // 4 * 3 = 12 bytes
  public maxs: vec3 = vec3.zero // 4 * 3 = 12 bytes
  public firstFace = -1 // 4 bytes
  public numFaces = 0 // 4 bytes
  public firstBrush = -1 // 4 bytes
  public numBrushes = 0 // 4 bytes

  public static TOTALBYTES = 40 // 12 + 12 + 16 = 40 bytes
}
...
export class Q3BspParser {
  ...
  public static readonly BSPID: number =
    ('P'.charCodeAt(0) << 24) +
    ('S'.charCodeAt(0) << 16) +
    ('B'.charCodeAt(0) << 8) +
    'I'.charCodeAt(0)
  ...
  public static toGLCoord(v: Q3BSPVertex, scale = 8): void {
