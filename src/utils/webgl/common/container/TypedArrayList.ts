export class TypedArrayList<T extends Uint16Array | Float32Array | Uint8Array> {
  ...
  public constructor(typedArrayConstructor: new (capacity: number) => T, capacity = 8) {
  ...
  public subArray(start = 0, end: number = this.length): T {
    return this._array.subarray(start, end) as T
  }

  public slice(start = 0, end: number = this.length): T {
