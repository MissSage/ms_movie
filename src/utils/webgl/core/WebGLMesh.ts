...
export class GLStaticMesh extends GLMeshBase {
  ...
  public constructor(
    ...
    const offsetMap: GLAttribOffsetMap = GLAttribState.getInterleavedLayoutAttribOffsetMap(
      this._attribState,
    )
    // 计算出顶点的数量
...
export class GLMeshBuilder extends GLMeshBase {
  // 字符串常量key
  private static SEQUENCED = 'SEQUENCED'
  private static INTERLEAVED = 'INTERLEAVED'
  ...
  public constructor(
    ...
    if (this._layout === EVertexLayout.INTERLEAVED) {
      ...
      const map: GLAttribOffsetMap = GLAttribState.getInterleavedLayoutAttribOffsetMap(
        this._attribState,
      )
      // 调用如下两个方法
  ...
  public color(r: number, g: number, b: number, a = 1.0): GLMeshBuilder {
    if (this._hasColor) {
      this._color.r = r
      this._color.g = g
      this._color.b = b
  ...
  public end(mvp: mat4): void {
    ...
    if (this._layout === EVertexLayout.INTERLEAVED) {
      ...
      const map: GLAttribOffsetMap = GLAttribState.getSequencedLayoutAttribOffsetMap(
        this._attribState,
        this._vertCount,
      )
