...
export class GLMatrixStack {
  ...
  public rotate(angle: number, axis: vec3, isRadians = false): GLMatrixStack {
    if (isRadians === false) {
...
export class GLWorldMatrixStack {
  ...
  public rotate(angle: number, axis: vec3, isRadians = false): GLWorldMatrixStack {
    if (isRadians === false) {
      angle = MathHelper.toRadian(angle)
    }
    this.modelViewMatrix.rotate(angle, axis)
