...
css-doodle {
    ...
    @grid: 1 / 8em; background: @shaders
      (
        texture1 {@grid: 1 / 100%; background: linear-gradient(
          ...
          ) ;} fragment {void main() {vec2 p=gl_FragCoord.xy / u_resolution.xy; vec2
          c=vec2(0.5, 0.5) ; vec2 uv=p.xy - c; float R=3.141593; float
          len=length(uv * vec2(u_resolution.x / u_resolution.y, 1)) ; float angle=atan(uv.y, uv.x) +
          R * smoothstep(0.75, 0, len) ; float r=length(uv) + .1; vec2
          coords=vec2(r * cos(angle), r * sin(angle)) + c; FragColor=texture(texture1, coords) ;}}
      )
      ;
  );
  ...
  --flowers: (
    @shape: clover 5; background: hsla(calc(360 - @i * 4), 70%, 68%, @r.8) ; transform:
